namespace recipes_backend.Controllers
{
    ...
    public class UsuariosController : ControllerBase
    {
        ...
        /// </returns>
        /// <param name="mail">Mail del usuario que quiere recuperar la contraseña</param>
        ...
        /// <response code="500">En el caso de haber un problema interno en el codigo</response>
        [HttpGet]
        [Route("recuperar-contraseña/{email}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> RecuperarContraseña([FromRoute, Required] string email)
        {
            if (!ModelState.IsValid)
                return BadRequest("Parametros enviados incorrectamente");

            await _usuarioService.RecuperarContraseña(email);
            return NoContent();
        }
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> ChequearCodigoValidacion([FromRoute, Required] string email, [FromBody] RecuperarContraseñaDTO recuperacionData)
        {
            if (!ModelState.IsValid)
                return BadRequest("Parametros enviados incorrectamente");

            await _usuarioService.ChequearCodigoValidacion(email, recuperacionData);
            return NoContent();
        }
        ...
        /// <response code="500">En el caso de haber un problema interno en el codigo</response>
        [HttpPatch]
        [Route("cambiar-contraseña/{email}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> CambiarContraseña([FromRoute, Required] string email, [FromBody] CambiarContraseñaDTO cambiarContraseñaData)
        {
            if (!ModelState.IsValid)
                return BadRequest("Parametros enviados incorrectamente");

            await _usuarioService.CambiarContraseña(email, cambiarContraseñaData);
            return NoContent();
        }
